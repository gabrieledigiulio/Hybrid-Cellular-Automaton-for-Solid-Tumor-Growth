...
### Simulation Cycle (per step)
```
Diffusion PDE → Apoptotic Removal → Agent Update (shuffle) → Resource Consumption → Data Collection
```
...
*(Where $D$ are the diffusion coefficients and $f$ represents the individual cell consumption/production rates).*
...
*(Where $T_r$ is the baseline target response and $k$ is the modulation multiplier).*
...
*(Where $p_i$ is the proportion of living cells belonging to the $i$-th subclone).*
...
> **Note**: `ffmpeg` must be installed on the system for MP4 video generation. On macOS: `brew install ffmpeg`. Without ffmpeg, videos are saved as GIFs.
...
### Files generated by `src/run_all.py`

| File | Content |
|---|---|
| `results/2d/<Experiment_Name>/data_*.npz` | Cell matrix for each step (compressed format) |
| `results/2d/<Experiment_Name>/*_counts.png` | Cell counts plot over time |
| `results/2d/<Experiment_Name>/*_metrics.png` | Shannon Index and Invasive Distance plot |
| `results/summary_table.csv` | Comparative table of all experiments |
| `src/video_generation/videos/*.mp4` | Animated videos of tumor growth |
